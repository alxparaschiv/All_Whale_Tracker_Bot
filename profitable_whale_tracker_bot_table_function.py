...
- Market sentiment summary with bullish/bearish bias
"""
...
class WhaleInfoBot:
    ...
    def get_market_bias(self, long_value, short_value):
        """Determine market bias based on long/short ratio"""
    ...
    def get_all_whale_positions(self):
        ...
        # Track market sentiment data
        market_data = {
            'BTC': {'long_count': 0, 'short_count': 0, 'long_value': 0, 'short_value': 0, 'whales': set()},
            'ETH': {'long_count': 0, 'short_count': 0, 'long_value': 0, 'short_value': 0, 'whales': set()},
            'SOL': {'long_count': 0, 'short_count': 0, 'long_value': 0, 'short_value': 0, 'whales': set()}
        }
        ...
        for whale in self.whales:
            ...
            # Skip whales with no positions for display
            if not positions:
                continue
            
            # Store for display later
            all_whale_positions.append({
                'whale': whale,
                'positions': positions
            })
            
            # Track ALL positions for market sentiment
            for pos in positions:
                coin = pos['coin']
                if coin in ['BTC', 'ETH', 'SOL']:
                    market_data[coin]['whales'].add(whale['address'])
                    
                    # Clear tracking based on side
                    if pos['side'] == 'LONG':
                        market_data[coin]['long_count'] += 1
                        market_data[coin]['long_value'] += pos['value']
                        logger.info(f"Added LONG {coin}: Count={market_data[coin]['long_count']}, Value={pos['value']}")
                    else:  # Must be SHORT since we only have two options
                        market_data[coin]['short_count'] += 1
                        market_data[coin]['short_value'] += pos['value']
                        logger.info(f"Added SHORT {coin}: Count={market_data[coin]['short_count']}, Value={pos['value']}")
        # Display positions for each whale
        for whale_data in all_whale_positions:
            whale = whale_data['whale']
            ...
            for pos in positions:
                ...
                # Add green/red circle for long/short
                side_emoji = "🟢" if pos['side'] == 'LONG' else "🔴"
                
                # Format position info
                message += f"{side_emoji} <b>{pos['coin']} {pos['side']}</b> {pnl_emoji}\n"
        ...
        if total_whales_with_positions == 0:
            ...
            message += f"<code>{'=' * 40}</code>\n"
            message += f"<b>📊 MAJOR ASSETS</b>\n"
            
            for coin in ['BTC', 'ETH', 'SOL']:
                data = market_data[coin]
                total_positions = data['long_count'] + data['short_count']
                if total_positions > 0:
                    bias, emoji = self.get_market_bias(data['long_value'], data['short_value'])
                    
                    # Format: BTC: BEARISH 1v6 (L:$500K S:$136.5M)
                    message += f"{emoji} <b>{coin}: {bias}</b> {data['long_count']}v{data['short_count']} "
                    message += f"(L:{self.format_value(data['long_value'])} S:{self.format_value(data['short_value'])})\n"
            
            message += "\n"
            
            # DETAILED MARKET SENTIMENT FOR EACH COIN
            for coin in ['BTC', 'ETH', 'SOL']:
                data = market_data[coin]
                total_coin_value = data['long_value'] + data['short_value']
                
                if total_coin_value > 0:
                    message += f"<code>{'=' * 40}</code>\n"
                    message += f"<b>📊 {coin} MARKET SENTIMENT</b>\n"
                    
                    # Calculate percentages
                    long_pct = (data['long_value'] / total_coin_value) * 100 if total_coin_value > 0 else 0
                    short_pct = (data['short_value'] / total_coin_value) * 100 if total_coin_value > 0 else 0
                    net_value = data['long_value'] - data['short_value']
                    
                    bias, emoji = self.get_market_bias(data['long_value'], data['short_value'])
                    
                    message += f"• Total: {self.format_value(total_coin_value)}\n"
                    message += f"• Long: {self.format_value(data['long_value'])} ({long_pct:.1f}%)\n"
                    message += f"• Short: {self.format_value(data['short_value'])} ({short_pct:.1f}%)\n"
                    
                    if net_value > 0:
                        message += f"• Net: Long {self.format_value(abs(net_value))}\n"
                    elif net_value < 0:
                        message += f"• Net: Short {self.format_value(abs(net_value))}\n"
                    else:
                        message += f"• Net: Balanced\n"
                    
                    message += f"• Bias: <b>{bias}</b> {emoji}\n"
                    message += f"• Active Whales: {len(data['whales'])}\n"
                    message += "\n"
